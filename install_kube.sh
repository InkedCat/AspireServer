...
install_traefik() {
  ...
  if ! kubectl wait --for=condition=available --timeout=60s deployment/traefik-deployment &>> "${LOG_FILE}"; then
    echo_warning "Traefik not healthy after 60s. Please check the logs to see what went wrong."
  else
